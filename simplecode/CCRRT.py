...
class CcrrtGo:
    ...
    def __init__(self, car, start, goal, rengong_tuoyuan, rand_area,ganzhi_tuoyuan,ganzhi_kuang): # 车型，起点，终点，

        self.car = car
        self.start = self.Node(start[0], start[1], start[2])
        self.end = self.Node(goal[0], goal[1], goal[2])
        self.rengong_tuoyuan = rengong_tuoyuan  # 人工椭圆
        self.ganzhi_tuoyuan = ganzhi_tuoyuan  # 感知椭圆
        self.ganzhi_kuang = ganzhi_kuang  # 感知框
    ...
    def steer(self, from_node, to_node):
        ...
        while self.calc_distance(prev, to_node) > self.dis_threshold and n_step < self.max_steer_step:
            ...
            if inter_node.cc < 1.0 - self.p_safe and self.in_place(inter_node) and self.safe_steer(inter_node):
    ...
    def get_chance_constrain(self, current):
        ...
        # cal for each obs
        for obs in self.rengong_tuoyuan:
            angle = abs(obs[4])
            angle = angle if angle <= math.pi / 2.0 else math.pi - angle  # 为什么要对角度进行约束？    计算外切矩形的最大x，y值
    ...
    def get_chance_constrain_from_pu(self, current):
        A, B = self.vehicle_constraints(current.x, current.y, current.yaw)
        delta_t = 0  # sum(min delte_tj)
        # cal for each obs
        for obs in self.ganzhi_tuoyuan:
            ...
            for a, b in zip(A, B):
                ...
                # sigma = current.conv + np.diag([obs[2], obs[3], obs[4]]) # 3*3
                erf_item = (a.dot(x).item() - b) / np.sqrt(2 * a.dot(sigma).dot(a.transpose()).item())
                cc_from_pu = 0.5 * (1 - erf(erf_item))
                if cc_from_pu < delta_tj:
                    delta_tj = cc_from_pu
            delta_t += delta_tj
        return delta_t
    ...
    def is_node_in_vehicle(self, node, vehicle):
        """
    ...
    def peng_zhuang_jian_ce_oie(self, ego_car_node, list_for_check):  # obs是障碍物列表，第二个参数也是节点，在函数内部我会将它转化为障碍物那样的列表
        ...
        for obs in list_for_check:
            ...
            if self.is_node_in_vehicle(jiao1,
                                       ego_car_obs):  # 这个函数第一个参数是节点，但是第二个参数应该是一个列表【障碍物的x，障碍物的y，障碍物矩形的长半，短半，yaw 】
                is_pengzhuang += 1
